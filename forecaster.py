...
COL_MAP = {
    'Дата': 'date', 'SKU': 'sku_id',
    'Выручка/день': 'revenue_total', 'Продажи/день': 'qty',
    'Цена': 'avg_price', 'Цена со скидкой маркетплейса': 'avg_price_discount',
...
def load_and_prepare(file_path_or_buffer) -> pd.DataFrame:
    ...
    if 'date' not in df.columns:
        raise ValueError(f"Нет колонки с датой. Есть: {list(df.columns)}")
    if 'revenue_total' not in df.columns:
        raise ValueError(f"Нет колонки выручки. Есть: {list(df.columns)}")
    ...
    median_diff = df_s['date'].diff().dt.days.median()
...
def build_features(df: pd.DataFrame, target: str = 'revenue_total') -> pd.DataFrame:
    d = df.copy()
    for col in ['avg_price', 'avg_price_discount', 'avg_price_base',
    ...
    if target in d.columns:
        ...
        d['target_yoy']  = d[target].shift(12)
    return d
...
def select_features(df: pd.DataFrame, target: str, top_n: int = 12) -> Tuple[List, Dict]:
    ...
    lgb.fit(X, y)
    total = lgb.feature_importances_.sum() + 1e-9
    imp = dict(zip(candidates, lgb.feature_importances_ / total))
    selected = sorted(imp, key=lambda x: -imp[x])[:top_n]
    return selected, imp


def _split(y, X, val_months=3):
    n = len(y)
...
def fit_lgbm(y, X, horizon, future_X) -> ModelResult:
    from lightgbm import LGBMRegressor
    ...
    preds = [max(0, float(np.expm1(m.predict(future_X.iloc[[min(i, len(future_X)-1)]].fillna(0))[0])))
    ...
    for q, lst in [(0.1, ci_l), (0.9, ci_h)]:
        ...
        for i in range(horizon):
            lst.append(max(0, float(np.expm1(
                qm.predict(future_X.iloc[[min(i, len(future_X)-1)]].fillna(0))[0]))))
    ...
    return ModelResult('LightGBM', np.array(preds), np.array(ci_l), np.array(ci_h),
                       _mape(y_val.values, val_pred), _rmse(y_val.values, val_pred), imp)


def fit_sarimax(y, X, horizon, future_X) -> Optional[ModelResult]:
    if len(y) < 24:
        return None
    ...
    val_pred = np.maximum(0, np.expm1(
        res.get_forecast(len(y_val), exog=X_val).predicted_mean.values))
    ...
    return ModelResult('SARIMAX',
                       ...
                       _mape(y_val.values, val_pred), _rmse(y_val.values, val_pred),
                       notes=f'order={order}')


def fit_ets(y, X, horizon, future_X) -> ModelResult:
...
def ensemble_forecast(sku_df: pd.DataFrame, target: str = 'revenue_total',
                      horizon: int = 3, scenario: Optional[Dict] = None) -> EnsembleResult:
    ...
    future_X = pd.DataFrame([X.tail(3).mean()] * horizon, columns=feature_cols, index=future_dates)
    ...
    for k, v_val in [('target_lag1', float(y.iloc[-1])),
                     ('target_ma3',  float(y.tail(3).mean())),
                     ('target_ma6',  float(y.tail(6).mean())),
                     ('target_ma12', float(y.tail(12).mean()))]:
        if k in feature_cols:
            future_X[k] = v_val

    if scenario:
        for k, v in scenario.items():
            for key in [f'{k}_lag1', k]:
                if key in future_X.columns:
                    future_X[key] = v
